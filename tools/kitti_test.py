...
for fold_config in fold_configs:
    ...
    if os.path.exists(base_gt_dir):
        ...
        for i, (img_path, gt_path, fname) in enumerate(tqdm(img_list, desc=f"{fold_config['name']} evaluating")):
        ...
        if preds and gts:
            ...
            print(f"total eval image: {len(preds)}")
            ...
            print(f"[{fold_config['name']}] error: no eval/gt pair to eval")

print("\n" + "="*80)
print("K-FOLD cross-val result")
print("="*80 + "\n")
...
if all_fold_metrics:
    ...
    if macc_values:
        ...
        print(f"\avg mPA: {avg_macc*100:.2f}%")
